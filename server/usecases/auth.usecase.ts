        ...
        try{
         ...
         let userType;
            ...
            if(!user && typeof body.password === 'string'){
                ...
                if(userType==='student'){
                    const {username , email,  gender, date_of_birth } = body as IStudentBody;
                    const studentDetails = await this.studentRepository.studentSignUp({username,email,password:hashedPassword,gender, date_of_birth});
                    if(studentDetails){
                        const otpResponse=await this.sendOTP({email})

                            return {
                        success:true,
                        message:"Student registered successfully, OTP sent for verification",
                    ...
                    const teacherDetails = await this.teacherRepository.teacherSignUp({username,email,password:hashedPassword,gender});
                    if(teacherDetails){
                        const otpResponse=await this.sendOTP({email})
                            return {
                        success:true,
       ...
       async userSignIn(body:IStudentBody | ITeacherBody | IAdminBody){
        try{
            ...
            if(user && isApproved && typeof body.password === 'string'){
                ...
                const matchPassword=await bcrypt.compare(body.password  ,user.password as string)
                 if(matchPassword){
               ...
               if(user && typeof body.password === 'string'){
                ...
                if(userType==='student'){
                    ...
                    const otpResponse=await this.sendOTP({email})
                         ...
                         await this.studentRepository.studentChangePassword(email,hashedPassword);
                    ...
                    const otpResponse=await this.sendOTP({email})
                    const hashedPassword:string = await  bcrypt.hash(password as string, 10);
                    await this.teacherRepository.teacherChangePassword(email,hashedPassword);
                    ...
                    return {
                        success:false,
                        message:"This email Id already used or this student is already exist "
                    }
