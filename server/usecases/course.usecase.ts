...
class CourseUsecase implements ICourseUsecaseInterface  {
    ...
    constructor(courseRepository: CourseRepository,conversationRepository: ConversationRepository) {
    ...
    async createCourse(courseData: ICourseBody, id: string) {
        try {
            ...
            const existingCourse = await this.courseRepository.findCourseByTitle(courseData.course_title);
            if (existingCourse) {
                return {
                    success: false,
            ...
            const newCourse:ICourse = await this.courseRepository.createCourse(courseData, id);
            if(newCourse){
              ...
              const studentsIds: ObjectId[] = newCourse?.students_list ?? [];
                const conversation = await this.conversationRepository.createNewCommunity(id,courseData.course_title ,studentsIds);
    ...
    async updateCourse(courseData: ICourseBody, id: string,courseId: string) {
        try {
            // If no course with the same title exists, proceed to create the new course
            const updatedCourse = await this.courseRepository.editCourse(courseData, id,courseId);
            return {
                success:true,
    ...
    async getCourses(courseData:ICourseBody){
        try {
            ...
            const { syllabus, standard}: { syllabus: string, standard: string } = courseData;

            const courseList = await this.courseRepository.getCourses(syllabus,standard);
    ...
    async getCoursesByTeacher(id:string){
        try {
            ...
            const courseList = await this.courseRepository.getCoursesByTeacher(id);
    ...
    async getCoursesById(id:string){
        try {
            console.log(id)
            const courseList = await this.courseRepository.findCourseById(id);
            if (courseList ) {
                return {
    ...
    async searchCourse(searchData: string) {
        try {
            const filteredCourses: ICourse[] = await this.courseRepository.searchCourse(searchData);
    ...
    async filterCourse(searchData: string) {
        try {
            const filteredCourses: ICourse[] = await this.courseRepository.filterCourse(searchData);
            if (filteredCourses && filteredCourses.length > 0) {
                return {
                    success: true,
                    message: "Courses found",
    ...
    async updateCourseStudentsList(courseId: string[], studentId: string) {
        try {
            ...
            const courseDetails= await this.courseRepository.updateCourseStudentsList(courseId,studentId);
            ...
            if(courseDetails ){
                ...
                for (const id of courseId) {
                    ...
                    if (course) {
                        ...
                        await this.conversationRepository.addMemberToConversation(communityId, studentId);
                    }
                }
                
                 return { success: true, message: 'Course students list updated successfully',courseDetails:courseDetails };
            }else{
                return { success: false, message: 'Course students list updation got  failed',courseDetails };
            }
        } catch (error) {
    ...
    async createReview(courseId: string, reviewData: IReviewBody) {
        try {
            ...
            const existingCourse = await this.courseRepository.findCourseById(courseId);
            ...
            const updatedCourse = await this.courseRepository.addReviewToCourse(courseId, newReview);
    ...
    async addMaterials(
        courseId: string,
        teacherId: string,
        materials: Materials // Use the Materials interface here
      ) {
        ...
        try {
          ...
          if (existingCourse.teacher_id.toString() !== teacherId) {
            return { success: false, message: "Unauthorized access. You are not the teacher of this course." };
          }
      
          // Add materials to the course
        
          const updatedCourse = await this.courseRepository.updateCourse(courseId,materials);
          console.log(updatedCourse)
          if(!updatedCourse){
              return { success: false, message: "Materials added  failed" }
              
            }
            return { success: true, message: "Materials added successfully.", updatedCourse };
        } catch (error) {
      ...
      async getMaterials(courseId: string) {
        try {
          ...
          const materials:any = await this.courseRepository.findMaterialsByCourseId(courseId);
          console.log(materials);
          if (materials && materials.length > 0) {
            const { _id, materials: extractedMaterials } = materials[0];
            // Return the materials associated with the course if it exists
            return { success: true, message: "Materials found", materials: extractedMaterials };
          } else {
            return { success: false, message: "No materials found for the provided course ID." };
          }
        } catch (error) {
      ...
      async approveCourse(id:string){
        try{
                const courseExist=await this.courseRepository.findCourseById(id);
                if(courseExist){
    
                    const Isapprovedcourse=await this.courseRepository.approveCourse(id)
      ...
      async disApproveCourse(id:string){
        try{
                ...
                if(courseExist){
                    ...
                    const Isapprovedcourse=await this.courseRepository.disApproveCourse(id)
                        return {
                            success:true,
                            message:"teacher approved successfully",
                            Isapprovedcourse,
...
async addSubject(syllabus: string, standard: string, subject: string): Promise<any> {
    try {
        // Check if the subject already exists for the provided syllabus and standard
        const existingSubject = await this.courseRepository.getSubjectByCriteria(syllabus, standard, subject);
        ...
        const newSubject = await this.courseRepository.addSubject(syllabus, standard, subject);
...
async getSubjects(syllabus: string, standard: string) {
    try {
      const subjects = await this.courseRepository.findSubjectsBySyllabusAndStandard(syllabus, standard);
      if (subjects && subjects.length > 0) {
        return { success: true, message: 'Subjects found', subjects };
      } else {
        return { success: false, message: 'No subjects found for the provided syllabus and standard.' };
      }
    } catch (error) {
