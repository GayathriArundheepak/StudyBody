...
class CourseUsecase implements ICourseUsecaseInterface {
  ...
  async createCourse(courseData: ICourseBody, id: string) {
    try {
      ...
      const newCourse: ICourse = await this.courseRepository.createCourse(
        courseData,
        id
      );
      if (newCourse) {
        ...
        const conversation =
          await this.conversationRepository.createNewCommunity(
            id,
            courseData.course_title,
            studentsIds
          );
  ...
  async updateCourse(courseData: ICourseBody, id: string, courseId: string) {
    try {
      // If no course with the same title exists, proceed to create the new course
      const updatedCourse = await this.courseRepository.editCourse(
        courseData,
        id,
        courseId
      );
  ...
  async getCourses(courseData: ICourseBody) {
    try {
      ...
      const courseList = await this.courseRepository.getCourses(
        syllabus,
        standard
      );
  ...
  async searchCourse(searchData: string) {
    try {
      const filteredCourses: ICourse[] =
        await this.courseRepository.searchCourse(searchData);
  ...
  async filterCourse(searchData: string) {
    try {
      const filteredCourses: ICourse[] =
        await this.courseRepository.filterCourse(searchData);
      if (filteredCourses && filteredCourses.length > 0) {
        return {
          success: true,
          message: "Courses found",
  ...
  async updateCourseStudentsList(courseId: string[], studentId: string) {
    try {
      const courseDetails =
        await this.courseRepository.updateCourseStudentsList(
          courseId,
          studentId
        );
      ...
      if (courseDetails) {
        for (const id of courseId) {
          ...
          if (course) {
            ...
            await this.conversationRepository.addMemberToConversation(
              communityId,
              studentId
            );
          }
  ...
  async createReview(courseId: string, reviewData: IReviewBody) {
    try {
      ...
      const updatedCourse = await this.courseRepository.addReviewToCourse(
        courseId,
        newReview
      );
  ...
  async getMaterials(courseId: string) {
    try {
      ...
      const materials: any =
        await this.courseRepository.findMaterialsByCourseId(courseId);
  ...
  async disApproveCourse(id: string) {
    try {
      const courseExist = await this.courseRepository.findCourseById(id);
      if (courseExist) {
        const Isapprovedcourse = await this.courseRepository.disApproveCourse(
          id
        );
        return {
          success: true,
  ...
  async addSubject(
    ...
    try {
      ...
      const existingSubject = await this.courseRepository.getSubjectByCriteria(
        syllabus,
        standard,
        subject
      );
      ...
      const newSubject = await this.courseRepository.addSubject(
        syllabus,
        standard,
        subject
      );
  ...
  async getSubjects(syllabus: string, standard: string) {
    try {
      const subjects =
        await this.courseRepository.findSubjectsBySyllabusAndStandard(
          syllabus,
          standard
        );
