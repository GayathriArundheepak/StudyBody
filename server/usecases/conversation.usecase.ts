...
class ConversationUsecase {
    ...
    async createNewConversation(senderId: string, receiverId: string) {
        try {
            ...
            const conversation = await this.conversationRepository.createNewConversation(senderId, receiverId);
            return {
                success:true,
    ...
    async findConversationByUserIds(firstUserId: string, secondUserId: string){
        try {
            const conversation = await this.conversationRepository.findConversationByUserIds(firstUserId, secondUserId);
            return{
                success:true,
