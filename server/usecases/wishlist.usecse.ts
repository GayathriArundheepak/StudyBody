...
class WishlistUsecase implements IWishlistUsecaseInterface {
    ...
    async addToWishlist(studentId:string,courseId:string) {
        try {
            const addedWishlist = await this.wishlistRepository.addToWishlist(studentId,courseId);
            return{
    ...
    async removeWishlist(studentId: string, courseId: string) {
        try {
            const removedWishlist = await this.wishlistRepository.removeFromWishlist(studentId, courseId);
