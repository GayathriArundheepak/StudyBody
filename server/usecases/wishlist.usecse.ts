...
class WishlistUsecase implements IWishlistUsecaseInterface {
  ...
  constructor(_wishlistRepository: WishlistRepository) {
    this._wishlistRepository = _wishlistRepository;
  }

  async addToWishlist(studentId: string, courseId: string) {
    try {
      const addedWishlist = await this._wishlistRepository.addToWishlist(
        studentId,
        courseId
      );
      return {
  ...
  async getWishlist(studentId: string) {
    try {
      ...
      const wishlist = await this._wishlistRepository.getWishlist(studentId);
  ...
  async removeWishlist(studentId: string, courseId: string) {
    try {
      const removedWishlist = await this._wishlistRepository.removeFromWishlist(
        studentId,
        courseId
      );
