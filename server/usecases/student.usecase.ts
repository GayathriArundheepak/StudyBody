...
class StudentUsecase implements IStudentUsecaseInterface {
    ...
    try {
      ...
      if (studentExist) {
        if (password) {
          ...
          const updateBody: IStudentBody = {
            ...body,
            password: hashedPassword,
          };
          const updatedDetails =
            await this._studentRepository.studentProfileUpdeate(updateBody, id);
  ...
  async getStudentById(id: string) {
    try {
      const student = await this._studentRepository.getStudentById(id);
      if (student) {
        return {
          success: true,
  ...
  async studentProfilePicUpload(file: Express.Multer.File, id: string) {
    try {
      ...
      if (studentExist) {
        const updatedPic =
          await this._studentRepository.studentProfilePicUpload(filePath, id);
        ...
        if (updatedPic) {
          ...
          return {
            success: false,
            message: "cant update pic",
          };
        }
      } else {
        return {
          success: false,
          message: "you not autherized to change ",
        };
      }
    } catch (error) {
      console.error(error);
  ...
  async updateStudentMylearning(studentId: string, courseIds: string[]) {
    try {
      const updatedStudent =
        await this._studentRepository.updateStudentMylearning(
          studentId,
          courseIds
        );
  ...
  async getMyLearningCourses(studentId: string) {
    try {
      const courses = await this._studentRepository.getMyLearningCourses(
        studentId
      );
      if (courses) {
        return {
          success: true,
          message: "My Learning courses found",
          data: courses,
        };
      } else {
  ...
  async getUsers(skip: number, limit: number) {
    try {
      ...
      const users = await this._studentRepository.getUsers(skip, limit);
