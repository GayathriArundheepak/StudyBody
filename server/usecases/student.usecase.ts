...
class StudentUsecase implements IStudentUsecaseInterface {
  ...
  async studentProfilePicUpload(file: Express.Multer.File, id: string) {
    try {
      ...
      if (studentExist) {
        const updatedPic = await this.studentRepository.studentProfilePicUpload(
          filePath,
          id
        );
  ...
  async updateStudentMylearning(studentId: string, courseIds: string[]) {
    try {
      const updatedStudent =
        await this.studentRepository.updateStudentMylearning(
          studentId,
          courseIds
        );
  ...
  async getMyLearningCourses(studentId: string) {
    try {
      const courses = await this.studentRepository.getMyLearningCourses(
        studentId
      );
      if (courses) {
        return {
