...
class CartUsecase implements ICartUsecaseInterface {
  ...
  async addToCart(studentId: string, courseId: string) {
    try {
      const addedCart = await this._cartRepository.addToCart(
        studentId,
        courseId
      );
      return {
  ...
  async getCart(studentId: string) {
    try {
      const cart = await this._cartRepository.getCart(studentId);
      if (!cart) {
        return {
          success: false,
  ...
  async removeFromCart(studentId: string, courseId: string) {
    try {
      const removedCart = await this._cartRepository.removeFromCart(
        studentId,
        courseId
      );
  ...
  async clearCart(studentId: string) {
    try {
      const cart = await this._cartRepository.clearCart(studentId);
      return {
        success: true,
