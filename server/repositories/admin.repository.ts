...
return adminDetails
    ...
    async adminChangePassword(email:string,password:string){
      try{
      await Admin.findOneAndUpdate({email:email},{$set:{password:password}},{new:true})
    ...
    async adminProfileUpdeate(updateBody:IAdminBody,id:string){
      ...
      const updated = await Admin.findByIdAndUpdate({_id:id},{$set:{username:username,email:email,password:password,profilePic:profilePic,date_of_birth:date_of_birth,gender:gender}},{new:true})
         return  updated ;
      }catch(error){
