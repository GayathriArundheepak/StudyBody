...
class CourseRepository implements ICourseRepositoryInterface {
  ...
  async getCourses(syllabus:string,standard:string) {
    try {
      const courseList = await Courses.find({syllabus:syllabus , standard:standard})
...
console.log('search');
        ...
        const regexPatterns: RegExp[] = searchWords.map(word => new RegExp(word, 'i')); // 'i' flag for case-insensitive search
        ...
        const filteredCourses: ICourse[] = courseList.filter(course => {
            ...
            return regexPatterns.every(regex => (
  ...
  async filterCourse(searchData: string) {
    try {
        ...
        const filteredCourses:ICourse[] = courseList.filter(course => {
          ...
          return (
              regexPattern.test(course.subject)  );
      });
  ...
  const promises = courseIds.map(async courseId => {
    try {
      ...
      if (  course.students_list && course.students_list.length >= 35) {
        return { success: false, message: `Course with ID ${courseId} is full. Cannot enroll more students.` };
      }
      ...
      if (updatedCourse.students_list && updatedCourse.students_list.length > 35) {
        return { success: false, message: `Course with ID ${courseId} is full. Cannot enroll more students.` };
      }

      return { success: true, message: `Course with ID ${courseId} students list updated successfully` };
    } catch (error) {
      throw new Error(`Error updating course with ID ${courseId} students list`);
    }
  });
...
async approveCourse(id:string){
  try{
   const isApproved= await Courses.findByIdAndUpdate({_id:id},{$set:{ adminApproved:true}},{new:true});
  return isApproved;
...
async disApproveCourse(id:string){
  try{
   const isApproved= await Courses.findByIdAndUpdate({_id:id},{$set:{ adminApproved:false}},{new:true});
  return isApproved;
...
async getSubjectByCriteria(syllabus: string, standard: string, subject: string){
  try {
      // Find a subject with the provided criteria
      const existingSubject = await Subjects.findOne({ syllabus, standard, subject });
...
async addSubject(syllabus: string, standard: string, subject: string): Promise<any> {
  ...
  async findSubjectsBySyllabusAndStandard(syllabus: string, standard: string){
    try {
      const subjects = await Subjects.find({ syllabus, standard }).distinct('subject');
