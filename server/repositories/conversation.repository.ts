...
class ConversationRepository{
    async createNewConversation(senderId: string, receiverId: string) {
      ...
      async  createNewCommunity(teacherId: string, course_title:string, studentIds: ObjectId[]) {
        try {
      ...
      async  getConversationsByUserId(userId: string): Promise<IConversation[]> {
        ...
        try {
          ...
          throw new Error(`Failed to get conversations for user ${userId}: ${error}`);
        }
      }
      async findConversationByUserIds(firstUserId: string, secondUserId: string): Promise<IConversation | null> {
        try {
          ...
          throw new Error(`Failed to find conversation between ${firstUserId} and ${secondUserId}: ${error}`);
        }
      }
