...
class TeacherRepository implements ITeacherRepositoryInterface {
  ...
  async otpApproval(email: string) {
    try {
      const updatedTeacher = await Teachers.findOneAndUpdate(
        { email: email },
        { $set: { otpApproved: true } },
        { new: true }
      );
      return updatedTeacher;
    } catch (error) {
      throw new Error(
        `Failed to update OTP approval status for teacher: ${error}`
      );
    }
  }
  ...
  async checktApproval(email: string) {
    try {
      ...
      if (teacher && teacher.otpApproved) {
        return true;
      } else {
        return false;
      }
  ...
  async teacherProfileUpdeate(updateBody: ITeacherBody, id: string) {
    try {
      ...
      const teacherDetails = await Teachers.findByIdAndUpdate(
        { _id: id },
        {
          $set: {
            username: username,
  ...
  async blockTeacher(id: string) {
    try {
      const isBlock = await Teachers.findByIdAndUpdate(
        { _id: id },
        { $set: { block: true } },
        { new: true }
      );
  ...
  async disApprovelTeacher(id: string) {
    try {
      const isApproved = await Teachers.findByIdAndUpdate(
        { _id: id },
  ...
  async teacherProfilePicUpload(filePath: string, id: string) {
    try {
      const updatedPic = await Teachers.findByIdAndUpdate(
        { _id: id },
