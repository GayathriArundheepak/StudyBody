...
class OrderController {
  ...
  constructor(_orderUseCase: OrderUseCase) {
  ...
  async createOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      const response = await this._orderUseCase.createOrder(ordersData);
  ...
  async getOrders(req: Request, res: Response): Promise<void> {
    try {
      const orders = await this._orderUseCase.getOrders();
  ...
  async findOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      const order = await this._orderUseCase.findOrder(orderId);
      if (!order) {
        res
  ...
  async updateOrderPaymentStatus(req: Request, res: Response) {
    try {
      ...
      const response = await this._orderUseCase.updateOrderPaymentStatus(
        orderId
      );
      const statusCode = response?.success
        ? HttpStatus.success
