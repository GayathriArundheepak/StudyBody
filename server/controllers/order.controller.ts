...
class OrderController {
    ...
    async findOrder(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!order) {
                res.status(HttpStatus.NotFound).json({ success: false, message: 'Order not found' });
            } else {
                res.status(HttpStatus.success).json({
                    success: true,
                    message: 'Order found',
    ...
    async updateOrderPaymentStatus(req: Request, res: Response) {
        try {
          ...
          const response = await this.orderUseCase.updateOrderPaymentStatus(orderId);
          const statusCode =response?.success? HttpStatus.success:HttpStatus.NotFound;
        console.log(response)
          ...
          res.status(HttpStatus.ServerError).json({ success: false, message: 'Error updating order payment status' });
        }
      }
