...
class StudentController implements IStudentControllerInterface {
    ...
    async studentProfileUpdate(req : Request , res : Response){
        ...
        try{
            ...
            const response= await this.studentUsecase.studentProfileUpdeate(req.body,id);
            ...
            async getStudents(req: Request, res: Response) {
                try {
                    console.log(req)
                    const response = await this.studentUsecase.getStudents();
            ...
            async getStudentById(req: Request, res: Response): Promise<void> {
                try {
                  ...
                  const response = await this.studentUsecase.getStudentById(studentId);
            ...
            async updateStudentMylearning(req: Request, res: Response) {
                try {
                    ...
                    const response = await this.studentUsecase.updateStudentMylearning(studentId,courseId);
                    const statusCode =response.success ? HttpStatus.success : HttpStatus.NotFound;
                    res.status(statusCode).send({
                        success: response.success,
                        message: response.message,
            ...
            async getMyLearningCourses(req: Request, res: Response) {
                try {
                  ...
                  const response = await this.studentUsecase.getMyLearningCourses(studentId);
              ...
              async getUsers(req: Request, res: Response) {
                try {
                    ...
                    const currentPage: number = parseInt(req.params.currentPage as string) || 2; // Default to page 1 if not provided
                    ...
                    const response = await this.studentUsecase.getUsers(skip, limit);
                    const statusCode =response.success? HttpStatus.success:HttpStatus.NotFound
