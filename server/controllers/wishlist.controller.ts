...
class WishlistController implements IWishlistControllerInterface {
  private _wishlistUsecase: WishlistUsecase;

  constructor(_wishlistUsecase: WishlistUsecase) {
    this._wishlistUsecase = _wishlistUsecase;
  }
  ...
  async addToWishlist(req: Request, res: Response) {
    try {
      ...
      const response = await this._wishlistUsecase.addToWishlist(
        studentId,
        courseId
      );
  ...
  async getWishlist(req: Request, res: Response) {
    try {
      ...
      // Call the use case to add the course to the wishlist
      const response = await this._wishlistUsecase.getWishlist(studentId);
  ...
  async removeWishlist(req: Request, res: Response) {
    try {
      ...
      // Call the use case to add the course to the wishlist
      const removedCourse = await this._wishlistUsecase.removeWishlist(
        studentId,
        courseId
      );
