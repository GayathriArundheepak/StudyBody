  ...
  const handleAddToWishlist = async () => {
        ...
        try {
            if (cartAdded) {
                if(currentUser ){
                    ...
                    api.delete(`/api/cart/remove-item/${studentId}/${courseId}`)
                        .then(response => {
    ...
    return (
        <div className='icon'>
            ...
            {/* Wishlist Icon */}
            <div 
            className="popup wishlistIcon" onMouseEnter={() => setShowWishlistPopup(true)} onMouseLeave={() => setShowWishlistPopup(false)} onClick={handleAddToWishlist}>
            <FavoriteBorderIcon  style={{ color: wishlistAdded ? '#fc036b' : 'black' }} />


                <span className={`popuptext ${showWishlistPopup ? 'show' : ''}`}> {wishlistAdded ? 'Remove ': 'wishlist'}</span>
            </div>

            {/* Cart Icon */}
        <div className="popup cartIcon" onMouseEnter={() => setShowCartPopup(true)} onMouseLeave={() => setShowCartPopup(false)} onClick={handleAddToCart}>
                <ShoppingCartOutlinedIcon style={{ color: cartAdded ? 'blue' : 'black' }} />
               
                <span className={`popuptext ${showCartPopup ? 'show' : ''}`}>{cartAdded ? 'Remove' : 'cart'}</span>
            </div> 
        </div>
    );
}
