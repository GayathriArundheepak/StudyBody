...
// setCurrentChat 
function ChatOnline({ onlineUsers,selectedCourseStudents,setCurrentChat}: ChatOnlineProps) {
  ...
  useEffect(() => {
    ...
    const handleClick = async (user: Friend) => {
      try {
        const res = await api.get(`/api/conversations/find/${currentId}/${user._id}`);
        ...
        if (res.data.conversations === null) {
          ...
          const createConversationRes = await api.post('/api/conversations', { senderId, receiverId });
          
          if (createConversationRes.data.success) {
            // If conversation creation is successful, fetch the conversation again
            const newRes = await api.get(`/api/conversations/find/${currentId}/${user._id}`);
    ...
    {onlineFriends.map((user) => (
      <div className='chatOnlineFriend' onClick={() => handleClick(user)}>
        <div className="chatOnlineImgContainer">
          <img className='chatOnlineImg' src={user?.profilePic ?? ''} alt="profile pic" />
          <div className="chatOnlineBadge"></div>
        </div>
        <span className='chatOnlineName'>{user._id === currentUser?._id ? 'You' : user.username}</span>
      </div>
    ))}
  </div>
