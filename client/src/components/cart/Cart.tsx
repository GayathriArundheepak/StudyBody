    ...
    const fetchCartItems = async () => {
      try {
        if (currentUser && currentUser._id) {
          const response = await api.get(`/api/cart/get-cart/${currentUser._id}`);
...
console.log('orderdata:',ordersData)
    try {
      api.post('/api/order/create-order', { ordersData})
      .then(response => {
  ...
  const handleRemoveItem = (itemId: string) => {
    if (currentUser) {
      ...
      api.delete(`/api/cart/remove-item/${userId}/${itemId}`)
        .then(response => {
        ...
        <div className="cart-table">
          ...
          {cartItems.map((course) => (
            <div key={course._id} className="cart-item">
              <div className="cell">{course.course_title} <br /><span>({course.description})</span></div>
