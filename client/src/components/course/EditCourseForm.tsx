...
const EditCourseForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.get(`/api/course/fetch-course/${courseId}`)
      .then(response => {
        setCourseData(response.data.courseList);
        // setFormData(response.data); // Set form data with fetched course data
      })
      .catch(error=> {
  ...
  const handleChangeStandard = (selectedStandard: string, setFieldValue: Function, { values }: FormikProps<any>) => {
    // Check if both syllabus and standard are selected
    if (values.syllabus && selectedStandard) {
      api.get(`/api/course/subjects/${values.syllabus}/${selectedStandard}`)
        .then((response) => {
    ...
    <div className="form">
      <Formik
           initialValues={{
            ...
            expiry: courseData.expiry || '',
            slot: {
              day: courseData.slot?.day || [],
              time: courseData.slot?.time || '',
               isWeekend: courseData.slot?.isWeekend || true,
            },
            prize: courseData.prize || '',
            commission: courseData.commission || '',
            promotion_video: courseData.promotion_video || '',
        ...
        validationSchema={Yup.object({
          ...
          slot: Yup.object().shape({
            day: Yup.array().min(1, 'At least one day must be selected').required('Days are required'),
            time: Yup.string().test('time-period', 'Invalid time (only One hour need).', (value) => {
              if (!value) return false;
              const [start, end] = value.split(' - ');
              const startTimeParts = start.match(/(\d+):(\d+) ([APap][mM])/);
              ...
              if (startAmPm === 'am' && endAmPm === 'pm' && endHour === 12) endHour = 24;
        ...
        onSubmit={(values, { setSubmitting }) => {
          ...
          axios
          .put(`http://localhost:8080/api/course/update-course/${id}/${courseId}`, values)
            .then((response) => {
        ...
        {(formik) => (
          <Form className="create-course-form form">
            ...
            <div className="title">
              ...
              <Field type="text" name="course_title" placeholder="eg: CBSE-std09-physics-May-2024-(yourcode)" className="input-field" />
              <ErrorMessage name="course_title" className="error-message" component="div" />
            </div>
            <div className="basic-info">
              <div className="syllabus">
                ...
                <ErrorMessage name="syllabus" className="error-message" component="div" />
              </div>
              <div className="standard">
                <label htmlFor="standard">Standard</label>
                <Field as="select" name="standard" className="input-field" onChange={(e: { target: { value: string; }; }) => handleChangeStandard(e.target.value, formik.setFieldValue, formik)}>
                ...
                <ErrorMessage name="standard" className="error-message" component="div" />
              </div>
              <div className="subject">
                ...
                <ErrorMessage name="subject" className="error-message" component="div" />
              </div>
              <div className="expiry">
                <label htmlFor="expiry">Course expiry</label>
                <Field type="date" name="expiry" placeholder="Mininum 12 months" className="input-field" />
                <ErrorMessage name="expiry" className="error-message" component="div" />
              </div>
            </div>
            <div className="slot">
              <div>
                <label htmlFor="slot.day">Previous-Days</label>
                <Field type="text" id="time" name="slot.day"className="input-field"></Field>
                {/* <label htmlFor="slot.day">Select-Days</label>
                <Select
                  options={days}
                  isMulti
                ...
                /> */}
                <ErrorMessage name="slot.day" className="error-message" component="div" />
              </div>
              <div>
                <label htmlFor="slot.time">Time</label>
                <Field type="text" id="time" name="slot.time" placeholder="1:00 PM - 2:00 PM" className="input-field" />
                <ErrorMessage name="slot.time" component="div" className="error-message" />
              </div>
              <div>
                <label htmlFor="slot.isWeekend">Is weekend?</label>
                <Field as="select" name="slot.isWeekend" id="isWeekend" className="input-field">
                  <option value="true">Yes</option>
                  <option value="false">No</option>
                </Field>
                <ErrorMessage name="slot.isWeekend" className="error-message" component="div" />
              </div>
            </div>
            <div className="finance">
              <label htmlFor="prize">Prize</label>
              <Field type="number" name="prize" placeholder="eg 2500 /-" className="input-field" />
              <ErrorMessage name="prize" className="error-message" component="div" />
              <label htmlFor="commission">Commission (%)</label>
              <Field type="number" name="commission" placeholder="eg. 20%" className="input-field" />
              <ErrorMessage name="commission" className="error-message" component="div" />
            </div>
            {/* <div>
              <label htmlFor="promotion_video">Promotion Video</label>
              <VideoUpload onVideoUrlChange={handleVideoUpload} />
              <ErrorMessage name="promotion_video" className="error-message" component="div" />
            </div> */}
            <div>
