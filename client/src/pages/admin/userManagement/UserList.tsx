...
function UserList({ userType }: UserListProps) {
  ...
  const onBlock = async (id: string) => {
    try {
      await axios.put(`http://localhost:8080/api/admin/block-user/${userType}/${id}`);
  ...
  const onUnblock = async (id: string) => {
    try {
      await axios.put(`http://localhost:8080/api/admin/unblock-user/${userType}/${id}`);
  ...
  const onDisapprovalApproval = async (id: string) => {
    try {
      await axios.put(`http://localhost:8080/api/admin/disApprove-teacher/${id}`);
      const updatedUsers = users.map(user => {
        if (user._id === id) {
                  ...
                  {user.block ? (
                    ...
                    <>
                      {userType === 'teacher' && !user.adminApproved && (
                        <button onClick={() => onAdminApprove(user._id)}>Approve</button>
                      )}
                      {user.adminApproved && userType === 'teacher' && (
                        <button onClick={() => onDisapprovalApproval(user._id)}>Disapprove</button>
                      )}
