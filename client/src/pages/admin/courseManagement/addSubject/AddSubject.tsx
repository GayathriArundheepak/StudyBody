...
const AddSubject: React.FC<AddSubjectProps> = () => {
  ...
  const onSubmit = async (values: any, { setSubmitting, resetForm }: any) => {
    ...
    try {
      ...
      api.post('/api/course/add-subjects', formattedValues)
      .then(response => {
        ...
        {({ isSubmitting, errors }) => ( // Pass errors object provided by Formik
          <Form>
            <div className="selection-container">
              ...
              <Field as="select" id="selectedSyllabus" name="selectedSyllabus">
                <option value="ICSE">ICSE</option>
                <option value="CBSE">CBSE</option>
                <option value="STATE">STATE</option>
              </Field>
              <ErrorMessage name="selectedSyllabus" component="div" className="error" />

              <label htmlFor="selectedStandard">Standard:</label>
              <Field as="select" id="selectedStandard" name="selectedStandard">
                {[...Array(12)].map((_, i) => (
              ...
              <ErrorMessage name="selectedStandard" component="div" className="error" />
            </div>
